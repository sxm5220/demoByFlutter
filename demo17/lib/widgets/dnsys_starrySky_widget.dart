...
class DnsysStarrySkyWidget extends StatefulWidget {
  const DnsysStarrySkyWidget({Key? key}) : super(key: key);

  @override
  State<DnsysStarrySkyWidget> createState() => _DnsysStarrySkyWidgetState();
}

class _DnsysStarrySkyWidgetState extends State<DnsysStarrySkyWidget>
  ...
  void _animateParticles(List<Particle> particlesList) {
    const maxDistance =
        60.0; // Maximum distance a particle can move from its initial position
    const double lerpFactor = 0.01;

    for (var particle in particlesList) {
      // Move particle towards target position
      double dx =
          (particle.targetPosition.dx - particle.position.dx) * lerpFactor;
      double dy =
          (particle.targetPosition.dy - particle.position.dy) * lerpFactor;
  ...
  List<Particle> _generateParticles(int count, {bool isBig = false}) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      final size =
          isBig ? Random().nextDouble() * 40 + 20 : Random().nextDouble() * 2;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: CustomPaint(
        painter:
            StarrySkyPainter(particles: particles, bigParticles: bigParticles),
        child: Container(),
      ),
    );
  }
...
class StarrySkyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var particle in particles) {
      ...
      canvas.drawCircle(
          particle.position, particle.size * particle.scale, paint);
    }
